	...
	public class MogRepositoryBrowserForm : System.Windows.Forms.Form
	{
		...
		private bool CreateMOGRepository(string repositoryPath)
		{
			...
			if (repositoryPath.Length != 0)
			{
				...
				{
					int dialogId = MOG_Progress.ProgressSetup("Creating MOG Repository", "Please wait while MOG creates the Repository.");
					MOG_Progress.ProgressInitGraphic(dialogId);
		...
		private string ExtractRepositoryPath(string path)
		{
			...
			if (File.Exists(iniFilename))
			{
				...
				if (ini.SectionExist("MOG_REPOSITORIES"))
				{
					...
					{
						string sectionName = ini.GetKeyNameByIndex("MOG_REPOSITORIES", 0);

...
namespace MOG_LicenseGenerator
{
	...
	{
		internal static bool Generate(string filename, string macAddress, int days, int clientLicenseCount)
		{
			return Generate(filename, macAddress, DateTime.Now.AddDays(days), clientLicenseCount);
		}

		internal static bool Generate(string filename, string macAddress, DateTime expiration, int clientLicenseCount)
		{
			return Generate(filename, macAddress, expiration, clientLicenseCount, MOG_TimeBomb.DisabledFeature_None);
		}

		internal static bool Generate(string filename, string macAddress, DateTime expiration, int clientLicenseCount, string disabledFeatureList)
		{

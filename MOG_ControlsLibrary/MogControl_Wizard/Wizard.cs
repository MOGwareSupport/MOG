...
namespace MOG_ControlsLibrary.Utils
{
	...
	/// are of the wizard.
	/// </summary>
    [Designer(typeof(MOG_ControlsLibrary.Utils.WizardDesigner))]
	[ToolboxItem(true)]
    [ToolboxBitmap(typeof(MOG_ControlsLibrary.Utils.Wizard))]
	public class Wizard : System.Windows.Forms.UserControl
	{
		private System.Collections.Stack mPageStack;
		private bool mPushPop = false;
		private bool mForwardPush = true;
		...
		/// <summary>
		/// Event called after next button is pressed.
		/// </summary>
		[Category("Action"), Description("This event will be fired every time the user clicks next")]
		public event EventHandler ClickNext;
		[Category("Action"), Description("This event will be fired after the user clicks next")]
		public event EventHandler NextClicked;
		/// <summary>
		/// Event called after back button is pressed.
		/// </summary>
		[Category("Action"), Description("This event will be fired every time the user clicks back")]
		public event EventHandler ClickBack;
		[Category("Action"), Description("This event will be fired after the user clicks back")]
		...
		[Category("Wizard"), Description("This event will be fired every time the user clicks back")]
		public bool PushPop
		{
			get { return mPushPop; }
			set { mPushPop = value; }
		...
		internal int PageIndex
		{
			...
			set
			{
				...
				//Select the new page
				ActivateNextPage(value);
			}
		}

		/// <summary>
		/// Alternative way of getting the current page by using wizardPage objects
		/// </summary>
...
//			}
		...
		private WizardPage Pop()
		{			
			return this.mPageStack.Pop() as WizardPage;
		}

		protected internal void ActivateNextPage(int index)
		{
			...
			ActivateNextPage(tWizPage);
		}

		protected internal int ActivateNextPage(WizardPage page)
		{
			// Validate the specified page
			int pageIndex = vPages.IndexOf(page);
			int nextPageIndex = page.OnValidateBeforeShow(this, pageIndex);
			// Check if the nextPageIndex is different than what was specified?
			if ((WizardPage)vPages[nextPageIndex] != page)
			{
				// Start drilling and validate this next page
				return ActivateNextPage((WizardPage)vPages[nextPageIndex]);
			}

			page.OnShowFromNext(this);

			// Activate the nextPageIndex
		...
		protected internal void ActivatePage(int index)
		{
			...
			//Change to the new Page
			WizardPage page = ((WizardPage) vPages[index]);
			...
			if (mPushPop && mForwardPush)
			{
				this.mPageStack.Push(vActivePage);
			}
		...
		{
			if (ClickNext != null)
				ClickNext(this, EventArgs.Empty);
			Next();
			if (NextClicked != null)
		...
		{
			if (ClickBack != null)
				ClickBack(this, EventArgs.Empty);

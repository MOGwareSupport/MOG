...
namespace EV.Windows.Forms
{
	...
	/// Provides date sorting
	/// </summary>
	public class ListViewStringSizeSort: ListViewTextSort
	{
		/// <summary>
		/// Constructor
		/// </summary>
		/// <param name="sortColumn">Column to be sorted</param>
		/// <param name="ascending">true, if ascending order, false otherwise</param>
		public ListViewStringSizeSort(Int32 sortColumn, Boolean ascending):
			base(sortColumn, ascending)
		{
		}
		...
		protected override Int32 OnCompare(String lhs, String rhs)
		{
			try
			{
				string lhsize = lhs.Trim("KBM ".ToCharArray());
				string rhsize = rhs.Trim("KBM ".ToCharArray());

				decimal lsize = Convert.ToDecimal(lhsize);
				decimal rsize = Convert.ToDecimal(rhsize);

				// Check for needed MegaByte conversion to kiloByte
				if (lhs.ToUpper().IndexOf("MB") != -1)
				{
					lsize = lsize * 10240;
				}

				if (rhs.ToUpper().IndexOf("MB") != -1)
				{
					rsize = rsize * 10240;
				}
				
				return Convert.ToInt32(lsize - rsize);
			}
			catch
			{
				return 1;
	...
	/// <summary>
	/// Provides file size sorting
	/// </summary>
	public class ListViewFileSizeSort: ListViewTextSort
	{
		/// <summary>
		/// Constructor
		/// </summary>
		/// <param name="sortColumn">Column to be sorted</param>
		/// <param name="ascending">true, if ascending order, false otherwise</param>
		public ListViewFileSizeSort(Int32 sortColumn, Boolean ascending):
			base(sortColumn, ascending)
		{
		}
		...
		protected override Int32 OnCompare(String lhs, String rhs)
		{
			...
			string newLhs = lhs.Replace(",", "");
			newLhs = newLhs.Replace(" KB", "");

			string newRhs = rhs.Replace(",", "");
			newRhs = newRhs.Replace(" KB", "");

			return Int32.Parse(newLhs, NumberStyles.Number) - Int32.Parse(newRhs, NumberStyles.Number);
		}
	}
    #endregion

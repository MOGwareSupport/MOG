...
namespace CodersLab.Windows.Controls
{
	...
	public class TreeView : System.Windows.Forms.TreeView
	{
		...
		/// <summary>
		/// Keeps track of node that has to be put in edit mode.
		/// </summary>
...
//				color[1]=tn.ForeColor;
		...
		public void RemoveNodeNotify(TreeNode node)
		{
			//We need to remove it from the selected nodes first because changing the other data members modifies the selected nodes
			if (SelectedNodes.Contains(node))
				SelectedNodes.Remove(node);

			if (tnMostRecentSelectedNode == node)
				tnMostRecentSelectedNode = null;

			if (tnSelectionMirrorPoint == node)
				tnSelectionMirrorPoint = null;
		}
	}

...
namespace Tst
{
		...
		,ICloneable
	{
		...
		/// <exception cref="NotSupportedException">The <see cref="TstDictionary"/> is read-only.</exception>
		/// <exception cref="NotSupportedException">The <see cref="TstDictionary"/> has a fixed size.</exception>
		public virtual void AddNoCase(String key, Object value)
		{
			if (key==null)
				throw new ArgumentNullException("key is null");
			if (key.Length==0)
			...
			// creating root node if needed.		
			if (Root == null)
				root = new TstDictionaryEntry(null,Char.ToUpper(key[0]));
			...
			while (i<key.Length)
			{
				...
				else
				{
					...
					if (p.EqChild==null)
						p.EqChild=new TstDictionaryEntry(p,Char.ToUpper(key[i]));
					p = p.EqChild;
				}
			}
			p.IsKey=true;
		...
		public virtual TstDictionaryEntry Find(String key) 
		{
			...
			{
				c = key[index];
				if (c < entry.SplitChar)
					entry = entry.LowChild;
				else if (c > entry.SplitChar)

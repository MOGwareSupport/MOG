...
{
	/// <summary>
	/// Summary description for MogForm_RepositoryBrowser_Server.
	/// </summary>
	public class MogForm_RepositoryBrowser_Server : System.Windows.Forms.Form
	{
		#region System defs
		...
		private void InitializeComponent()
		{
			...
			System.Resources.ResourceManager resources = new System.Resources.ResourceManager(typeof(MogForm_RepositoryBrowser_Server));
			...
			this.tvDiskTree.AfterSelect += new System.Windows.Forms.TreeViewEventHandler(this.tvDiskTree_AfterSelect);
			...
			this.tvReposView.AfterSelect += new System.Windows.Forms.TreeViewEventHandler(this.tvReposView_AfterSelect);
			// 
			// cmRepositoryView
			// 
			...
			// 
			// MogForm_RepositoryBrowser_Server
			// 
			...
			this.Name = "MogForm_RepositoryBrowser_Server";
		...
		private string selectedIniPath = "";
		#endregion
		#region Properties
		...
		{
			get { return this.selectedIniPath; }
		}
		...
		public bool RepositoryViewVisible
		{
			...
			set	
			{
				if (this.RepositoryViewVisible != value  &&  this.Visible)
					SetRepositoryViewVisible(value); 
			}
		}
		...
		#endregion
		#region Constructors
		public MogForm_RepositoryBrowser_Server()
		{
			InitializeComponent();
		...
		private bool PerformRepositoryCreation(string path, bool confirm)
		{
			...
			creationSuccess = CreateMOGRepository( path );
			Cursor.Current = Cursors.Default;

			if (creationSuccess)
			{
				MOG_Prompt.PromptMessage("Creation Confirmation", "New MOG Repository created at " + path, Environment.StackTrace, MOG_ALERT_LEVEL.MESSAGE);
				return true;
			}
			else
			{
				MOG_Report.ReportMessage("Creation Failed", "Failed to create a MOG Repository at " + path, Environment.StackTrace, MOG_ALERT_LEVEL.ERROR);
				return false;
			}
		}
		...
		private bool CreateMOGRepository(string repositoryPath)
		{
			...
			if (repositoryPath.Length != 0)
			{
				...
				{
					MOG_Report.ReportMessage("Access Denied", "The specified repository is read only.", Environment.StackTrace, MOG_ALERT_LEVEL.CRITICAL);
					return false;
				}
				...
				if (File.Exists(rootRepositoryFilename))
				{
					...
					if (rootIni.SectionExist("MOG_REPOSITORIES"))
					{
						...
						{
							existingRepositoryNames.Add(rootIni.GetKeyNameByIndexSLOW("MOG_REPOSITORIES", i));
						}
					}
					rootIni.Close();
				}
				...
				do
				{
					...
					if (repositoryName == "")
					{
						MOG_Report.ReportMessage("Invalid Name", "Please enter a name for the new MOG Repository", Environment.StackTrace, MOG_ALERT_LEVEL.ALERT);
						bSuccess = false;
					}
					else
					{
						...
						foreach (string reposName in existingRepositoryNames)
						{
							...
							{
								MOG_Report.ReportMessage("Name Exists", "A MOG Repository of the name you specified already exists", Environment.StackTrace, MOG_ALERT_LEVEL.ALERT);
								bSuccess = false;
								break;
							}
						}
				...
				if (Directory.Exists(blankReposDir))
				{
					...
					args.Add(rootRepositoryFilename);
					args.Add(blankReposDir);

					ProgressDialog progress = new ProgressDialog("Creating MOG Repository", "Please wait while MOG creates a default repository.", CreateMogRepository_Worker, args, true);
				...
				{
					MOG_Report.ReportMessage("Missing Source Repository", "Couldn't find '" + blankReposDir + "'", Environment.StackTrace, MOG_ALERT_LEVEL.ERROR);
					bFailed = true;
				}
			}
			else
			{
				MOG_Report.ReportMessage("Creation Failed", "Invalid Repository Path Specified", Environment.StackTrace, MOG_ALERT_LEVEL.ERROR);
				bFailed = true;
			}
		...
		private void CreateMogRepository_Worker(object sender, DoWorkEventArgs e)
		{
			BackgroundWorker worker = sender as BackgroundWorker;
			List<string> args = e.Argument as List<string>;
			string repositoryName = args[0];
			string repositoryPath = args[1];
			string rootRepositoryFilename = args[2];
			string blankReposDir = args[3];
			...
			{
				MOG_Report.ReportMessage("Copy Error", "Couldn't copy '" + blankReposDir + "'", Environment.StackTrace, MOG_ALERT_LEVEL.ERROR);
				e.Cancel = true;
			}
		}
		#endregion

		private string ExtractRepositoryPath(string path)
		{
			string returnPath = path;

			string iniFilename = path.Trim("\\".ToCharArray()) + "\\MogRepository.ini";
			if (File.Exists(iniFilename))
			{
				// try to extract a repository path
				MOG_Ini ini = new MOG_Ini(iniFilename);
				if (ini.SectionExist("MOG_REPOSITORIES"))
				{
					if (ini.CountKeys("MOG_REPOSITORIES") > 0)
					{
						string sectionName = ini.GetKeyNameByIndexSLOW("MOG_REPOSITORIES", 0);
						if (ini.SectionExist(sectionName))
						{
							if (ini.KeyExist(sectionName, "SystemRepositoryPath"))
							{
								returnPath = ini.GetString(sectionName, "SystemRepositoryPath");
							}
						}
					}
				}
		...
		private string ExtractRepositoryConfigFile(string path)
		{
			string returnPath = path.Trim("\\".ToCharArray()) + "\\" + MOG_Main.GetDefaultSystemRelativeConfigFileDefine();		// default to {path}\Tools\mogConfig.ini
			...
			if (File.Exists(iniFilename))
			{
				...
				if (ini.SectionExist("MOG_REPOSITORIES"))
				{
					if (ini.CountKeys("MOG_REPOSITORIES") > 0)
					{
						...
						{
							if (ini.KeyExist(sectionName, "SystemRepositoryPath")  &&
								ini.KeyExist(sectionName, "SystemConfiguration"))
							{
								returnPath = ini.GetString(sectionName, "SystemConfiguration").Replace( "{SystemRepositoryPath}", ini.GetString(sectionName, "SystemRepositoryPath") );
							}
						}
					}
				}
		...
		private bool ContainsMogRepository_Shallow(string path)
		{
			...
			{
				string sectionName = ini.GetKeyNameByIndexSLOW("MOG_REPOSITORIES", i);
				if (ini.SectionExist(sectionName))
				{
					if (ini.KeyExist(sectionName, "SystemRepositoryPath"))
					{
		...
		private void tvDiskTree_MouseDown(object sender, System.Windows.Forms.MouseEventArgs e)
		{
			...
			tvDiskTree_AfterSelect(this, new TreeViewEventArgs(rtn));
		}
		...
		private void tvReposView_MouseDown(object sender, System.Windows.Forms.MouseEventArgs e)
		{
			...
			if (rtn != null  &&  rtn.IsINI)
			{
				if (rtn.Parent != null  &&  (rtn.Parent is rTreeNode))
				{
					this.selectedPath = ((rTreeNode)rtn.Parent).DiskPath;
				}

				this.selectedIniPath = Path.GetFileName(rtn.DiskPath);
			}
		}
		...
		private void tvReposView_KeyUp(object sender, System.Windows.Forms.KeyEventArgs e)
		{
			if (e.KeyData == Keys.F5)
				...
				this.RepositoryViewVisible = !this.RepositoryViewVisible;
		}
		...
		private void miRemoveFolder_Click(object sender, System.EventArgs e)
		{
			...
			if (MessageBox.Show("WARNING: You are about to delete " + rtn.DiskPath + " and all its sub-folders and files.\nAre you sure you want to proceed?", "WARNING", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation) == DialogResult.Yes)
			{
				...
				{
					MOG_Report.ReportMessage("Error", "Couldn't delete " + rtn.DiskPath, Environment.StackTrace, MOG_ALERT_LEVEL.ERROR);
					return;
				}
			}
		}
		...
		private void tvDiskTree_AfterLabelEdit(object sender, System.Windows.Forms.NodeLabelEditEventArgs e)
		{
			...
			try
			{
				...
				foreach (rTreeNode subNode in parent.Nodes)
				{
					if (subNode.Text.ToLower() == label.ToLower())
						this.tvDiskTree.SelectedNode = subNode;
				}
			}
			catch
			{
				MOG_Report.ReportMessage("Error", "Couldn't delete " + path, Environment.StackTrace, MOG_ALERT_LEVEL.ERROR);
				e.CancelEdit = true;
				rtn.Remove();
			}
			finally
		...
		private void miClean_Click(object sender, System.EventArgs e)
		{
			...
			if (File.Exists(path))
			{
				...
				{
					MOG_Report.ReportMessage("Error", "Couldn't clean " + rtn.DiskPath, Environment.StackTrace, MOG_ALERT_LEVEL.ERROR);
					return;
				}
			}

			MOG_Report.ReportMessage("Clean Complete", "Cleaned " + rtn.DiskPath, Environment.StackTrace, MOG_ALERT_LEVEL.ALERT);
		...
		private void tvReposView_AfterSelect(object sender, System.Windows.Forms.TreeViewEventArgs e)
		{
			rTreeNode rtn = e.Node as rTreeNode;
		...
		private void tvDiskTree_AfterSelect(object sender, System.Windows.Forms.TreeViewEventArgs e)
		{
			rTreeNode rtn = e.Node as rTreeNode;

			if (rtn != null)
			{
				this.selectedPath = ExtractRepositoryPath(rtn.DiskPath);
				this.selectedIniPath = Path.GetFileName( ExtractRepositoryConfigFile(rtn.DiskPath) );

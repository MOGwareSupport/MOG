...
{
	/// <summary>
	/// Summary description for MogForm_RepositoryBrowser_ServerManager.
	/// </summary>
	public class MogForm_RepositoryBrowser_ServerManager : System.Windows.Forms.Form
	{
		#region System defs
		...
		private void InitializeComponent()
		{
			...
			System.Resources.ResourceManager resources = new System.Resources.ResourceManager(typeof(MogForm_RepositoryBrowser_ServerManager));
			...
			// 
			// MogForm_RepositoryBrowser_ServerManager
			// 
			...
			this.Name = "MogForm_RepositoryBrowser_ServerManager";
		...
		#endregion
		#region Constructors
		public MogForm_RepositoryBrowser_ServerManager()
		{
			InitializeComponent();
		...
		private bool PerformRepositoryCreation(string path, bool confirm)
		{
			...
			if (confirm  &&  MOG_Prompt.PromptResponse("Confirm Repository Creation", 
				"Are you sure you want to create a new MOG Repository at " + path + "?", MOGPromptButtons.YesNo) != MOGPromptResult.Yes)
			{
				return false;
			}
			...
			{
				MOG_Prompt.PromptResponse("Creation Confirmation", "New MOG Repository created at " + path);
				Cursor.Current = Cursors.Default;
				return true;
			}
			else
			{
				MOG_Prompt.PromptResponse("Creation Failed", "Failed to create a MOG Repository at " + path);
				Cursor.Current = Cursors.Default;
				return false;
			}
		}
		...
		private bool CreateMOGRepository(string repositoryPath)
		{
			...
			if (repositoryPath.Length != 0)
			{
				...
				{
					MOG_Prompt.PromptResponse("Access Denied", "The specified repository is read only.");
					return false;
				}
				...
				while (true)
				{
					...
					if (repositoryName == "")
					{
						MOG_Prompt.PromptResponse("Invalid Name", "Please enter a name for the new MOG Repository");
						continue;
					}
					...
					foreach (string reposName in existingRepositoryNames)
					{
						...
						{
							MOG_Prompt.PromptResponse("Name Exists", "A MOG Repository of the name you specified already exists");
							continue;
						}
					}
				...
				if (Directory.Exists(blankReposDir))
				{
					...
					{
						MOG_Prompt.PromptResponse("Copy Error", "Couldn't copy '" + blankReposDir + "'");
						bFailed = true;
					}
				}
				else
				{
					MOG_Prompt.PromptResponse("Missing Source Repository", "Couldn't find '" + blankReposDir + "'");
					bFailed = true;
				}
			...
			{
				MOG_Prompt.PromptResponse("Creation Failed", "Invalid Repository Path Specified");
				bFailed = true;
			}
		...
		private void miRemoveFolder_Click(object sender, System.EventArgs e)
		{
			...
			if (MOG_Prompt.PromptResponse("WARNING", "WARNING: You are about to delete " + rtn.DiskPath + " and all its sub-folders and files.\nAre you sure you want to proceed?", MOGPromptButtons.YesNo) == MOGPromptResult.Yes)
			{
				try
				{
				...
				{
					MOG_Prompt.PromptResponse("Error", "Couldn't delete " + rtn.DiskPath, "", MOGPromptButtons.OK, MOG_ALERT_LEVEL.ERROR);
					return;
				}
			}
		}
		...
		private void tvDiskTree_AfterLabelEdit(object sender, System.Windows.Forms.NodeLabelEditEventArgs e)
		{
			...
			{
				MOG_Prompt.PromptResponse("Error", "Couldn't create " + path, "", MOGPromptButtons.OK, MOG_ALERT_LEVEL.ERROR);
				e.CancelEdit = true;
				rtn.Remove();
			}
			finally
		...
		private void miClean_Click(object sender, System.EventArgs e)
		{
			...
			if (MOG_Prompt.PromptResponse("Confirm Clean", "Are you sure you want to clean " + rtn.DiskPath, MOGPromptButtons.YesNo) == MOGPromptResult.No)
				return;
			...
			if (File.Exists(path))
			{
				...
				{
					MOG_Prompt.PromptResponse("Error", "Couldn't clean " + rtn.DiskPath, "", MOGPromptButtons.OK, MOG_ALERT_LEVEL.ERROR);
					return;
				}
			}

			MOG_Prompt.PromptResponse("Clean Complete", "Cleaned " + rtn.DiskPath, "", MOGPromptButtons.OK, MOG_ALERT_LEVEL.MESSAGE);

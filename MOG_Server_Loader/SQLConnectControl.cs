...
namespace MOG_Server_Loader
{
	...
	public class SQLConnectControl : System.Windows.Forms.UserControl
	{
		...
		private void SQLTestButton_Click(object sender, System.EventArgs e)
		{
			...
			// Attempt to open a connection to make sure we will work?
			SqlConnection myConnection = new SqlConnection(this.mConnectString);
			try
			{
				myConnection.Open();
			}
			...
			{
				ex.ToString();
				MessageBox.Show("Couldn't connect to SQL database " + this.SQLDatabaseComboBox.Text + " on server " + SQLServerComboBox.Text, "Connection Failure");
				return;
			}
			finally
			{
			...
			MessageBox.Show("Successfully connected to SQL database " + this.SQLDatabaseComboBox.Text + " on server " + SQLServerComboBox.Text, "Connection Successful");

...
namespace MOG_CoreControls
{
	...
	public class MOG_XpProgressBar : Control
	{
		#region "  Constructor  "

		private const string CategoryName = "Xp ProgressBar";
		
		public MOG_XpProgressBar()
		{}
		...
		private DrawStyle mDrawStyle = DrawStyle.Normal;
		private int mMarqueeAnim = 0;
		private Timer mMarqueeTimer;
		...
		[Category(CategoryName)]
		[Description("The Draw Style of the gradient bar in Progress Bar")]
		[DefaultValue(DrawStyle.Normal)]
		public DrawStyle DrawStyle
		{
			...
			set
			{
				if (mDrawStyle != value)
				{
					...
					if (mDrawStyle == DrawStyle.Marquee)
					{
						mPosition = 100;
						mMarqueeAnim = 0;
						mMarqueeTimer = new Timer();
						mMarqueeTimer.Interval = 100;
						mMarqueeTimer.Tick += new EventHandler(mMarqueeTimeer_Tick);
						mMarqueeTimer.Enabled = true;
						mMarqueeTimer.Start();
					}
					else
					{
						if (mMarqueeTimer != null)
						{
							mMarqueeTimer.Stop();
							mMarqueeTimer.Enabled = false;
							mMarqueeTimer = null;
						}							
					}
				}
